...
class KathaParser:
    ...
    def parse(self):
        ...
        for number, block in mantra_blocks:
            unit_id = f"KATHA_{number.replace('.', '_')}"
            ...
            yaml_match = re.search(r"```yaml(.*?)```", translation, re.DOTALL)
            unit_metadata = {}
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Parse Kaṭha Upaniṣad Markdown to JSON")
    arg_parser.add_argument("input", help="Path to katha.md")
    arg_parser.add_argument("output", help="Output JSON file")

    args = arg_parser.parse_args()

    parser = KathaParser(args.input)
    parser.parse()
