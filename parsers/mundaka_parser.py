...
class MundakaParser:
    ...
    def parse(self):
        ...
        for number, block in mantra_blocks:
            unit_id = f"MUNDAKA_{number.replace('.', '_')}"
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Parse Muṇḍaka Upaniṣad Markdown to JSON")
    arg_parser.add_argument("input", help="Path to mundaka.md")
    arg_parser.add_argument("output", help="Output JSON file")

    args = arg_parser.parse_args()

    parser = MundakaParser(args.input)
    parser.parse()
